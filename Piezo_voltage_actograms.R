...
rms <- read.csv("4WeekPhotoperiod_15Apr2019_RMS_Day1-9.csv")
rms <- read.csv("2WeekAcclimation_26Mar2019_RMS_Day1-7.csv")
...
for(i in unique(agg.rms_hour$Individual)) {
  #windows()
  #png(file=paste0(i, "_4wk.png"),width = 1500, height=954)
  ggplot(agg.rms_hour[agg.rms_hour$Individual==i,], aes(as.factor(Hour), Day)) + my_theme + 
  ...
  scale_y_reverse(breaks=unique(agg.rms_hour$Day)) + ggtitle(paste0(i, "_", agg.rms_hour$Treatment)) +
  theme(plot.title = element_text(hjust=0.5), axis.text.x = element_text(size=15))
  ggsave(paste0(i, "_4wk.png"),width = 10.5, height=6)
  #dev.off()
}
