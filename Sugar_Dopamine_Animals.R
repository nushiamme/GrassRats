...
### TO DO: sugar consumption in grams of sugar for dopamine
## Line plots including unreasonable values for phase 1 & 2
...
cols_dop <- c("grey60", "grey80", "yellow", "red", "red", "yellow", "black")


## Take out dopamine animals separately
conc_dop <- conc_all[!is.na(conc_all$DopamineStage),]
conc_dop$Fed_sugar_amt_g <- conc_dop$Fed_sugarsoln_g_perday*conc$Sugar_conc 

## Order levels in the Dopamine column
conc_dop$DopamineStage <- factor(conc_dop$DopamineStage, levels=c("Pre", "HCS", "HCS_Surgery", "DopLO1","DopHI2", "DopHI1","DopLO2", "Saline"))
conc_dop$IndivSexPhotoperiod <- paste0(conc_dop$Indiv, "_", conc_dop$Sex, "_", conc_dop$Treatment)
conc_dop$IndivSexPhotoperiod <- factor(conc_dop$IndivSexPhotoperiod, levels=c("G6_F_Short", "T35_F_Short", "T37_F_Short",
...
conc_8_all_dop <- conc_dop[conc_dop$Pre_test=="Test" & conc_dop$Sugar_conc==0.08,]
conc_8_all_dop$Prop_sugar <- conc_8_all_dop$Fed_sugarsoln_g_perday/
  (conc_8_all_dop$Fed_sugarsoln_g_perday+conc_8_all_dop$Fed_water_g_perday)
levels(conc_8_all_dop$Treatment)[levels(conc_8_all_dop$Treatment)=="Long"] <- "Neutral"
conc_8_dop_good <- conc_8_all_dop[conc_8_all_dop$Sugar_mmt_good=="Y" & conc_8_all_dop$Water_mmt_good=="Y",]


### Histograms of sugar and water consumed to determine "reasonable" cut-offs
ggplot(conc_8_all_dop[!is.na(conc_8_all_dop$Fed_sugarsoln_g_perday),], aes(Fed_sugarsoln_g_perday)) + geom_histogram(aes(fill=Sugar_mmt_good)) + 
...
ggplot(conc_8_sugar, aes(Treatment, Fed_sugar_amt_g)) + 
  facet_grid(.~Sugar_conc, scales="free_x") +
  my_theme + geom_boxplot(aes(fill=Treatment)) + #ylim(0,75) +
...
prop_sugar_boxPhase_all <- ggplot(conc_8_all_dop[!is.na(conc_8_all_dop$Prop_sugar),], aes(DopamineStage, Prop_sugar)) + my_theme + 
...
prop_sugar_indivs_day_all <- ggplot(conc_8_all_dop[!is.na(conc_8_all_dop$Prop_sugar),], aes(DaySinceStart, Prop_sugar)) + my_theme + 
  geom_line(aes(group=IndivSexPhotoperiod), show.legend = F, size=1) + 
...
## Fed sugar amount in grams
Amt_sugar_indivs_day_all <- ggplot(conc_8_all_dop[!is.na(conc_8_all_dop$Fed_sugar_amt_g),], aes(DaySinceStart, Fed_sugar_amt_g)) + my_theme + 
  geom_line(aes(group=IndivSexPhotoperiod), show.legend = F, size=1) + 
  geom_point(aes(fill=DopamineStage), col="black", size=4, pch=21) +
  facet_wrap(.~IndivSexPhotoperiod, scales = "free_x") +
  scale_fill_manual(values = cols_dop) +
  theme(axis.text.x = element_text(angle=90, size=20, vjust=0.5), legend.text = element_text(size=20),
        legend.key.height = unit(3,"line")) +
